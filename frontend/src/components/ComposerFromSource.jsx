...
// One source (a blog post, transcript, RSS item, raw notes) rewritten
// natively for several platforms at once — folded in from the standalone
// Repurpose page. Picking one of the results applies it to the post
// currently open here instead of navigating to a new one.
export function ComposerFromSource({ onApply }) {
  ...
  return (
    <div data-testid="composer-source-panel">
      ...
      <textarea data-testid="repurpose-source" value={source} onChange={(e) => setSource(e.target.value)} rows={6}
      ...
      <div className="mt-4 flex flex-wrap items-center gap-2">
        <ModelPicker value={model || defaultModel} onChange={setModel} models={models} testid="repurpose-model" className="w-auto min-w-[160px] flex-none" />
      ...
      {(loading || Object.keys(results).length > 0) && (
        <div className="mt-4 grid gap-3 sm:grid-cols-2">
          {loading && selected.map((k) => (
            <div key={k} className="generating-pulse rounded-xl border border-lime/40 bg-[#0A0A0A] p-4">
          ...
          {!loading && Object.entries(results).map(([k, text]) => {
            ...
            return (
              <div key={k} className="flex flex-col rounded-xl border border-white/10 bg-[#0A0A0A] p-4" data-testid={`repurpose-result-${k}`}>
                ...
                <div className="mt-3 flex gap-2">
                  ...
                  <Button onClick={() => onApply({ content: text, platform: k })}
                    className="h-8 flex-1 gap-1.5 rounded-lg bg-lime px-3 text-xs font-semibold text-[#0A0A0A] hover:bg-lime-hover" data-testid={`repurpose-schedule-${k}`}><Send size={14} /> Use this</Button>
                </div>
              </div>
            );
          })}
